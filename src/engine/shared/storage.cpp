...
public:
	...
	void FindDataDir()
	{
		...
		// 3) check for usable path in argv[0]
		{
			...
			str_append(aBaseDir, "/data/fonts", sizeof(aBaseDir));
		...
		{
			const char *aDirs[] = {
				"/usr/share/teesolar/data",
				"/usr/share/games/teesolar/data",
				"/usr/local/share/teesolar/data",
				"/usr/local/share/games/teesolar/data",
				"/usr/pkg/share/teesolar/data",
				"/usr/pkg/share/games/teesolar/data",
				"/opt/teesolar/data"
			};
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "%s/fonts", aDirs[i]);
				if(fs_is_dir(aBuf))
				{
